...
RC Table::drop()
{
  ...
  if (rc != RC::SUCCESS)  // 刷新失败
    return rc;

  // 删除.table原数据文件
  std::string meta_path = table_meta_file(base_dir_.c_str(), name());
  if (unlink(meta_path.c_str()) != 0)  // 将文件从文件系统中删除,参数为要删除的文件的路径名
  {
    LOG_ERROR("Failed to remove meta file=%s, error=%d",meta_path.c_str(),errno);
    return RC::GENERIC_ERROR;  // 表示一般性的错误
  }

  // 删除.data数据文件
  std::string data_path = table_data_file(base_dir_.c_str(), name());
  if (unlink(data_path.c_str()) != 0)  // 将文件从文件系统中删除,参数为要删除的文件的路径名
  {
    LOG_ERROR("Failed to remove data file=%s, error=%d",data_path.c_str(),errno);
    return RC::GENERIC_ERROR;  // 表示一般性的错误
  }
  ...
  for (int i = 0; i < index_num; i++) {
    ((BplusTreeIndex *)indexes_[i])->close();            // 先隐式转换为B+树的索引
    const IndexMeta *index_meta = table_meta_.index(i);  // 返回第i个索引
    std::string      index_file = table_index_file(base_dir_.c_str(), name(), index_meta->name());
    if (unlink(index_file.c_str()) != 0) {
      LOG_ERROR("Failed to remove index file=%s, error=%d",index_file.c_str(),errno);
      return RC::GENERIC_ERROR;  // 表示一般性的错误
    }
  }
